...
def search_for_meter(request):
    if request.is_ajax():
        ...
        query = "select id, consumer, address, '{2}' serial, '{3}' brand from zanecometerpy.consumers where consumer like '{0}' and address like '{1}' ".format(
            name, address, serial, brand)
